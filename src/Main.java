...
public class Main {
    ...
    private static final int cpMessageThreshold = 20 * tickPerSecond;
    ...
    private FileData ControlPointMessages(int i) {
        ...
        for (Team team : teams) {
            ...
            // Increment counter if team is on CP and message is not sent
            fileCommands.add(execute.As(new Entity("@a[gamemode=!spectator,team=" + team.getName() + "]"), false) +
                    execute.IfNext(new Entity("@s[gamemode=!spectator,x=" + (controlPoints.get(i - 1).getCoordinate().getX() - 6) + ",y=" + (controlPoints.get(i - 1).getCoordinate().getY() - 1) + ",z=" + (controlPoints.get(i - 1).getCoordinate().getZ() - 6) + ",dx=12,dy=12,dz=12,scores={MSGDum1CP" + i + "=.." + cpMessageThreshold + "}]"), true) +
                    "scoreboard players add @a[team=" + team.getName() + "] MSGDum1CP" + i + " 1");

            // Define CP under attack message
            ArrayList<TextItem> texts = new ArrayList<>();
            texts.add(new Text(Color.light_purple, false, false, "TEAM "));
            texts.add(new Text(team.getColor(), false, false, team.getJSONColor()));
            texts.add(new Text(Color.light_purple, true, false, " IS ATTACKING CONTROL POINT " + i + "!"));
    ...
    private FileData TraitorCheck() {
        ...
        fileCommands.add(execute.Unless(new Entity("@a[limit=1,tag=Traitor,gamemode=!spectator]")) +
    ...
    private FileData TeamsAliveCheck() {
        ArrayList<String> fileCommands = new ArrayList<>();
        for (int i = 0; i < teams.size(); i++) {
            fileCommands.add(execute.Unless(new Entity("@a[limit=1,team=!" + teams.get(i).getName() + ",gamemode=!spectator]")) +
        ...
        for (int i = 0; i < teams.size(); i++) {
            for (int j = 1; j < 3; j++) {
                fileCommands.add(execute.If(new Entity("@a[limit=1,scores={Admin=1,Victory=1}]"), false) +
                        ...
                        callFunction("" + FileName.victory_message_ + i));
                fileCommands.add(execute.If(new Entity("@a[limit=1,scores={Admin=1,Victory=1}]"), false) +
                        execute.IfNext(new Entity("@p[team=" + teams.get(i).getName() + ",gamemode=!spectator,scores={ControlPoint" + j + "=" + (maxCPScore * tickPerSecond) + "..},tag=Traitor]")) +
                        execute.UnlessNext(new Entity("@p[team=" + teams.get(i).getName() + ",gamemode=!spectator,scores={ControlPoint" + j + "=" + (maxCPScore * tickPerSecond) + "..},tag=!Traitor]"), true) +
                        callFunction(FileName.victory_message_traitor));
            }
        }
